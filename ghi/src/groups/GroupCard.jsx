...
const GroupImage = ({ group, fetchWithCookie }) => {
    const [image, setImage] = useState(null);

    useEffect(() => {
        const fetchImage = async () => {
        const Response = await fetchWithCookie(
            `${process.env.REACT_APP_SAMPLE_SERVICE_API_HOST}/files/${group.group_img}`
        );
    ...
    return (
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {groups?.map(group => {
                    return (
                        <div className="flex items-center justify-center mt-5" key={group.id}>
                            ...
                            <div className="pb-6">
                                ...
                                <h3 className="mb-1 text-2xl font-bold leading-normal text-black-700 dark:text-black-300 mt-5">
                                    {group.group_name}
                                </h3>
                                <br />
